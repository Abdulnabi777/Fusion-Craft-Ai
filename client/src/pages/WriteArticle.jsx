...
const WriteArticle = () => {
  ...
  return (
    <div className='h-full overflow-y-scroll p-6 flex justify-center items-start gap-4 text-slate-700'>
      {/* Left Coulum*/}
      <form onSubmit={onSubmitHandler} className='w-full max-w-lg p-4 bg-white rounded-lg border border-gray-200'>
        ...
        <div className='mt-3 flex gap-3 flex-wrap sm:max-w-9/11'>
          {articleLength.map((item, index)=>(
            ...
            className={`text-xs px-4 py-1 border rounded-full cursor-pointer ${selectedLength.text === item.text ? 'bg-blue-100 text-blue-700 border-blue-400' : 'text-gray-500 border-gray-300'}`}>
              {item.text}
            </span>
          ))}
        </div>
